<html>
<head>
<title>Mobile Detection :: Primitive </title>
<script>
</script>
</head>
<body>

<?php 

/*			 	  
				  Module 		:: Idigitalise Share Project 
				  Description	:: This script will detect mobile OS and direct to specific links on the fly.
				  Name   		:: Nitin Babar
				  Date   		:: 14th June 2013
*/

	$ua = $_SERVER["HTTP_USER_AGENT"];
	
	//print_r($ua);
	
	$android = strpos($ua, 'Android') ? true : false;			// Android
	$blackberry = strpos($ua, 'BlackBerry') ? true : false;		// BlackBerry
	$iphone = strpos($ua, 'iPhone') ? true : false;				// iPhone
	$palm = strpos($ua, 'Palm') ? true : false;					// Palm
	
	$linux = strpos($ua, 'Linux') ? true : false;				// Linux
	$mac = strpos($ua, 'Macintosh') ? true : false;				// Macintosh
	$win = strpos($ua, 'Windows') ? true : false;				// Windows
    $chrome = strpos($ua, 'Chrome') ? true : false; 			// Google Chrome

	// Firefox
	$firefox = strpos($ua, 'Firefox') ? true : false; // All Firefox
	$firefox_2 = strpos($ua, 'Firefox/2.0') ? true : false; // Firefox 2
	$firefox_3 = strpos($ua, 'Firefox/3.0') ? true : false; // Firefox 3
	$firefox_3_6 = strpos($ua, 'Firefox/3.6') ? true : false; // Firefox 3.6

	// Internet Exlporer
	$msie = strpos($ua, 'MSIE') ? true : false; // All Internet Explorer
	$msie_7 = strpos($ua, 'MSIE 7.0') ? true : false; // Internet Explorer 7
	$msie_8 = strpos($ua, 'MSIE 8.0') ? true : false; // Internet Explorer 8

	// Opera
	$opera = preg_match("/\bOpera\b/i", $ua); // All Opera


	// Safari
	$safari = strpos($ua, 'Safari') ? true : false; // All Safari
	$safari_2 = strpos($ua, 'Safari/419') ? true : false; // Safari 2
	$safari_3 = strpos($ua, 'Safari/525') ? true : false; // Safari 3
	$safari_3_1 = strpos($ua, 'Safari/528') ? true : false; // Safari 3.1
	$safari_4 = strpos($ua, 'Safari/531') ? true : false; // Safari 4

	if ($ua) {

	// ---- Test if using a Handheld Device ----
	if ($android) { // Android
	echo 'You are using an Android. We are redirecting to Google Play in 5 seconds.Please wait.
	';
	sleep(5);
	header('Location:https://play.google.com/store');
	}
	if ($blackbery) { // Blackbery
	echo 'You are using a Blackbery.We are redirecting to Appstore  in 5 seconds.Please wait.
	';
	sleep(5);
	header('Location:http://store.apple.com/us');
	}
	if ($iphone) { // iPhone
	echo 'You are using an iPhone.We are redirecting to Appstore  in 5 seconds.Please wait.
	';
	sleep(10);
	header('Location:http://store.apple.com/us');
	}
	if ($palm) { // Palm
	echo 'You are using a Palm.We are redirecting to Appstore  in 5 seconds.Please wait.
	';
	sleep(5);
	header('Location:http://store.apple.com/us');
	}

	}
?>
</body>
</html>